...
async def stream_generate_content(
    ...
    """
    【修改版】Antigravity 路由：強制使用假流式以過濾思考鏈
    """
    ...
    async def fake_stream_generator():
        ...
        try:
            ...
            # 在這裡清洗 content
            if content:
                log.debug(f"[ANTIGRAVITY-FILTER] Original len: {len(content)}")
                content = clean_thinking_text(content)
                log.debug(f"[ANTIGRAVITY-FILTER] Cleaned len: {len(content)}")
